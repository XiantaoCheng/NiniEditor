...
class Debugger(QTextEdit):
    ...
    def initialize(self,point):
        ...
        pt_compiler=tools_basic.getPoint(point,'m_compiler','compiler')
        if pt_compiler.m_dev==None:
            self.m_compiler=Compiler(pt_compiler)
        else:
    ...
    def control(self,key):
        if key==Qt.Key_S:
            self.m_compiler.run(1)
        elif key==Qt.Key_W:
            self.m_compiler.runAll()
        elif key==Qt.Key_A:
    ...
    def setSentsForRun(self,code):
        ...
        self.m_compiler.reset()
        karmas=self.m_compiler.compile(code)
        self.m_compiler.loadCode(karmas)
        ...
        self.markWords()
        self.m_compiler.run(0,karmas[0])
    ...
    def movePtr(self,right):
        ...
        if right==False:
            if ptr<=0:
                ptr=0
            elif self.m_compiler.m_running==[] or self.m_compiler.m_running[-1]!=curKm:
                pass
    ...
    def setRunningSent(self,ptr):
        ...
        self.m_ptrKm=ptr
        self.m_compiler.retrive(curKm)
        self.m_compiler.run(0,curKm)
        

    def reset(self,code='',IO=None):
        self.m_words.clear()
        self.m_sents.clear()
        self.m_compiler.reset()
        if IO!=None:
            self.m_compiler.m_source=IO
