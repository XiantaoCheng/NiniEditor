...
+[保存文本](,body_compiler)
    ...
    def tmpPtsToOutputs(self):
        list_pop=[]
        for pt in self.m_tmpNew:
            try:
                if pt.m_stateProj==True:
                    self.m_outputs.append(pt)
                    list_pop.append(pt)
            except:
                pass
        for pt in list_pop:
            self.m_tmpNew.remove(pt)
    ...
    def termBuiltin(self,question):
        ...
        elif question.m_name=='[投影]':
            return self.tpProject(obj,question)
        elif question.m_name=='[新]':
            return self.tpNew(obj,question)
    ...
    def tpProject(self,obj,question):
        if question.m_db[1]==None:
            return False
        if question.m_text=='实节点' and obj.m_name[0]=='[' and obj.m_name[-1]==']':
            obj.m_name=obj.m_name[1:-1]
            obj.m_needed=None
        obj.m_stateProj=True
        self.answerQuestion(question)
        return True

    def tpNew(self,obj,question):
        if question.m_db[1]==None:
            return False
        if obj in self.m_tmpNew:
            self.answerQuestion(question)
            return True
        return False
    ...
    def runCode_test(self,code,question=None,IO=None,limit=False,list_pt=None):
        tmp=None
        tmp_r=None
        source=None
